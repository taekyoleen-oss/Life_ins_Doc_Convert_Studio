...
export const FORMULA_SAMPLES: FormulaSample[] = [
  ...
  { group: "기초", label: "연납 환산 납입기수", text: "N* = k · [ ( N′_x − N′_{x+m} ) − ( k−1 )/( 2·k )·( D′_x − D′_{x+m} ) ]" },
  { group: "유지자수", label: "탈퇴 사유 1개", text: "l_{x+t+1} = l_{x+t} × ( 1 − q_{x+t} )" },
  { group: "유지자수", label: "탈퇴 사유 2개", text: "l_{x+t+1} = l_{x+t} × ( 1 − q_{x+t} − r_{x+t} + q_{x+t}·r_{x+t}/2 )" },
  ...
  { group: "계산기수", label: "C (진단·발생)", text: "C_{x+t} = l_{x+t}·r_{x+t}·v^{t+½}" },
  ...
  { group: "보험료", label: "영업보험료 (산출방법서형)", text: "G = [ P + ( α_S + α_P·P_base )·D′_x/N* + β_S/k + β′·( N_{x+m} − N_{x+n} )/N* ] / ( 1 − β_G − γ )" },
