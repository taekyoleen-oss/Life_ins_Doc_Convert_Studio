...
- 📦 **패키지 · 최근 작업** — [패키지 → 패키지로 저장]은 조건(YAML)·산출방법서(Word·Markdown)·MethodSpec JSON·위험률 표(CSV)를 한 파일 `.lidpkg`(압축하지 않은 ZIP — 이름을 .zip 으로 바꾸면 안의 파일을 꺼낼 수 있다)로 저장한다. 위험률 표가 없어도 된다. [패키지 열기]·[열기]·끌어다 놓기로 되살리고, 저장·연 것은 [최근 작업]에 남아 한 번에 불러온다. 첫 화면과 [샘플]도 조건·산출방법서·위험률 표(가상의 값)가 한 세트다.
