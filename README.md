...
- 🗂️ **조건 입력 카드** — 자유설계보험(flexible_insurance) 빌더와 같은 단계 카드(M01 상품 기본정보 · M02 계약조건 · M03 이자율·저해지 · M04 위험률 · M05 납입자수 · C01… 담보 · M06 사업비 · M07 책임준비금·해지환급금 · M08 수식 더하기). YAML 을 몰라도 칸을 채우고 ＋ 로 담보·위험률·사업비 행을 더한다. M01 에는 산출방법서에 싣는 **가입 조건**(보험의 종류·보험종목·보험기간·납입기간·가입나이·납입주기·가입금액 한도·갱신 — 사업방법서 판매 범위 표 모양)을, M02 에는 실제로 계산하는 **시산 기준** 한 점을 적는다. [YAML] 탭은 같은 조건의 파일 보기 — 칸을 고치면 그 값만 파일에 들어가 주석·줄 맞춤이 남는다.
