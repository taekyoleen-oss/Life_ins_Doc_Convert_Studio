...
- 📊 **위험률 표** — Excel 표 붙여넣기·CSV·TSV·XLSX 를 올리면 첫 행을 열 이름으로 읽고, 열마다 연령·위험률·성별로 조건에 잇는다(이름이 겹치면 자동). 이은 열은 산출방법서 위험률 표와 MethodSpec JSON 에 실린다 — 남·여 열이 있으면 두 벌 다(`RateRef.tables`), 자유설계보험이 계약정보 성별의 표를 고른다.
